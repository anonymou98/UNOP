# models/gala/graph_encoder.py
"""
GALA Graph Encoder: For unstructured point cloud inputs (3D)

Adaptive Multiscale Graph (AMG) encoder with physics token aggregation.
"""
...
class PhysicsTokenBlock(nn.Module):
    """Physics Token Aggregation Block"""
    
    def __init__(self, feature_width, num_heads=8, num_phys=32, dropout=0., **kwargs):
        super(PhysicsTokenBlock, self).__init__()
    ...
    def single_in(self, x):
        ...
        phy_weights = self.softmax(self.l_phy(phy_x) / self.temperature)
        
        phy_norm = phy_weights.sum(1).unsqueeze(-1)
        phy_norm = (phy_norm + 1e-5).repeat(1, 1, self.feature_width)
    ...
    def forward(self, x, batch):
        ...
        if batch is not None and batch.max() > 0:
            ...
            for x_single in x_batch:
                phy_token, phy_weights = self.single_in(x_single)
            ...
            x = self.single_out(phy_y.squeeze(0), phy_weights)
...
class MultiscaleGraphBlock(nn.Module):
    def __init__(self, feature_width, input_features=1, output_features=1, 
        ...
        self.num_phys = num_phys
        self.phy_aggr = PhysicsTokenBlock(feature_width, num_heads=num_heads, num_phys=num_phys, dropout=0.0)
...
class GALAGraphEncoder(nn.Module):
    """
    GALA Graph Encoder (AMG - Adaptive Multiscale Graph)
    
    For point cloud feature extraction with physics-aware token aggregation.
    """
    ...
    def __init__(self, feature_width, num_layers, 
        ...
        super(GALAGraphEncoder, self).__init__()
        ...
        self.output_norm = nn.LayerNorm(output_features)
